...
- **Same priority threads are scheduled cooperatively.** The scheduler itself is tickless, therefore time-slicing isn't supported. Timers are still supported through the timer API from the integrated [Embassy] crate.
...
- Multiple **asynchronous Tasks** can be spawned within each thread with an executor from the integrated [Embassy] crate. This bridges the gap to async Rust, future-based concurrency, and asynchronous I/O. The executor executes all its tasks inside the thread context. When all tasks on the executor are pending, the owning thread is suspended.
...
- **Condition variables** are stateless and thread-independent. Any thread can wait or signal on a shared condition variable. When a signal is sent, either one or all of the threads that are waiting at that moment are notified.
...
- **Supported dual-core Chips** where SMP is enabled by default:
    - RP2040
    - ESP32S3
    - RP2350 (Work-in-Progress)
- **Porting from single-core to multi-core** requires no changes in the user-application.
- **One global runqueue** is shared among all cores. The scheduler assigns the _n_ highest priority, ready, and non-conflicting thread to the _n_ available cores. The scheduler can be invoked individually on each core. Whenever a higher priority thread becomes ready, the scheduler is triggered on the core with the lowest priority running thread to perform a context switch.
...
- **On multi-core** an additional hardware spinlock is used in case of the RP2040, and atomics in case of the ESP32S3 to ensure that the critical section is enforced on all cores.
